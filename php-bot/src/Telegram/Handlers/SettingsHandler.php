    ...
    public static function startEditTemplate(int $chatId, int $messageId, int $userId, BotContext $ctx): void
    {
        ...
        $ctx->telegram->editMessageText(
            ...
            "قالب جدید سیگنال را بفرستید. از این متغیرها استفاده کنید:\n{$placeholders}\n\n"
                . 'اگر پرمیوم دارید، می‌تونید ایموجی سفارشی رو مستقیم هرجای متن (جلو یا پشت) بچسبونید - خودش ذخیره و حفظ می‌شه.',
            Keyboards::cancel('templates')
        );
    }

    /** @param array<int, array<string, mixed>> $entities */
    public static function receiveTemplate(int $chatId, int $userId, string $text, array $entities, BotContext $ctx): void
    {
        AdminStateService::clear($userId);
