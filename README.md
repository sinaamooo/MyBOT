...
- **تست خودکار آفلاین** با ۵۱ بررسی (`tools/selftest.php`) و بازرسی چیدمان (`tools/layout-check.php`).
...
**۱) مستقیم (ساده‌ترین راه)** — اگر حساب خودتان پریمیوم است، کافی است ایموجی پریمیوم را همان‌جا داخل متن کپشن بگذارید و بفرستید. لازم نیست شناسه‌ای پیدا کنید: تلگرام ایموجی را به‌صورت entity از نوع `custom_emoji` می‌فرستد، ربات شناسه را خودش برمی‌دارد و عیناً در کانال بازتولیدش می‌کند. قالب‌بندی‌های دیگر (بولد، نقل‌قول و …) هم سر جایشان می‌مانند.
